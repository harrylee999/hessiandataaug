...
class hessian():
    ...
    def dataloader_hv_product(self, v):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.data):
            ...
            loss = self.criterion(outputs, targets.to(device).long())
            loss.backward(create_graph=True)
            params, gradsH = get_params_grad(self.model)
            self.model.zero_grad()
    ...
    def trace_layer(self, maxIter=100, tol=1e-3):
               
        device=self.device
        traces_vhv=[] #返回all layer 值

        # gradsH = [self.gradsH[i:i+2] for i in range(0, len(self.gradsH), 2)]
        # params = [self.params[i:i+2] for i in range(0, len(self.params), 2)]
